...
//		    echo $directory.' is not a valid path';die();
//		}
            ...
            if (!$json) {
                ...
                foreach ($files as $file) {
                    ...
                    if (!$json) {
                        $s3->upload($s3Config['bucket-name'], $directory . '/' . $filename, fopen($file['tmp_name'], 'rb'), 'public-read');
                    }
                }
            }
...
//            }
            ...
            if ($this->request->server['REQUEST_METHOD'] == 'POST') {
                ...
                $folderExistsInS3 = $s3->doesObjectExist($bucket, $directory . '/' . $folder.'/');
...
//                }
            ...
            if (!isset($json['error'])) {
                $directoryCreationResult = $s3->putObject(array(
                    'Bucket' => $bucket,
                    'Key' => $directory . '/' . $folder.'/index.html',
                    'Body' => "",
                    'ACL'          => 'public-read',
                    'ContentLength' => 0
                ));
